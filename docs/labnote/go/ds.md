...
正常来说，在分布式环境下，**重试 + 幂等**是保证业务成功的重要手段。
...
应该说，在整个分布式系统中，大多数的可用性问题、一致性问题和业务可靠性问题，基本上都可以通过**重试加幂**等来解决。如果要是重试之后都失败了，那就只能考虑手工介入了。
...
1. 并发更新：这种不一致是很好的解决的，不管使用版本号来控制，还是用分布式锁来控制，都很好解决。

2. 部分失败，遇到这种部分失败的问题，完全没有办法解决。最多就是引入重试，但是引入重试之后重试也可能失败。
...
分布式事务是指在分布式系统中，涉及多个节点的事务。它需要保证这些节点上的操作要么全部成功，要么全部失败，以确保数据的一致性和完整性
