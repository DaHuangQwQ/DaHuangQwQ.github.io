...
在整个过程里面还有很多细节。比如说，如果要是 P 执行 Goroutine 的时候又产生了新的 Goroutine（比如说你使用了 go 关键字开 goroutine），这个时候会优先放回去 P 自己的队列里面。放回自己的队列可以充分**利用局部性原理**
